      ...
      <div className="fixed right-0 top-0 h-full w-full sm:w-96 bg-background z-50 shadow-2xl flex flex-col animate-slide-in-right">
        ...
        {cart.length > 0 && (
          <div className="border-t border-border p-6 space-y-4">
            ...
            {/* WhatsApp Order */}
            <a
              href={`https://wa.me/923001234567?text=Hi! I want to order:%0A${cart.map(item => `${item.name} (${item.quantity}x) - Rs. ${item.price * item.quantity}`).join('%0A')}%0A%0ATotal: Rs. ${getTotalPrice()}`}
              target="_blank"
